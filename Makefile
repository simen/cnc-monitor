...
flash:	all
	$(AVRDUDE) -U flash:w:verifier.hex:i
...
load: all
	bootloadHID verifier.hex

clean:
	rm -f verifier.hex main.elf $(OBJECTS)

# file targets:
main.elf: $(OBJECTS)
	$(COMPILE) -o main.elf $(OBJECTS) -lm

verifier.hex: main.elf
	rm -f verifier.hex
	avr-objcopy -j .text -j .data -O ihex main.elf verifier.hex
	avr-size *.hex *.elf *.o
